function downloadBaiduTiles(){
	...
	function getFullTiles(map,level,callback){
		var  levels = [3,4,5,6,7];
		var levelPlus = [1,2,4,10,20,40,80,100,200,400,1000,2000,4000,10000,20000,40000];
		console.log(levelPlus);
		var zoomLevel = map.getZoom();
		zoomLevel = level;
		var center = convertLL2MC(map.getCenter());
		var width = map.width * levelPlus[level-3]/200;
		var height = map.height * levelPlus[level-3]/200;
		console.log(width,height);
		var cV = Math.pow(2, 18-zoomLevel);
		var unitSize = cV * 256;
		var longitudeUnits = Math.ceil(center.lng / unitSize);
		...
		var left = cP[0] - Math.ceil((width / 2 - cP[2]) / tileSize);
		var top = cP[1] - Math.ceil((height / 2 - cP[3]) / tileSize);
		var right = cP[0] + Math.ceil((width / 2 + cP[2]) / tileSize);
		var c0 = 0;
		var bottom = cP[1] + Math.ceil((height / 2 + cP[3]) / tileSize) + c0;
		var xydata = [];
